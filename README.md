...
Runge-Kutta is a more accurate method, as we will see. It involves taking multiple evaluations across the time step of length h. The improved Euler method is a simple example of this. Instead of using $f(u_i,t_i)$ as the derivative, we use $f(u_i+(h/2)f(u_i,t_i), t_i + h/2)$. The derivation of IE2 is the same, approximate the derivative using the same forward difference, and then solve for $u_{i+1}$. 
